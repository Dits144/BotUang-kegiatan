...
function handleTodo(ctx, canManage) {
  ...
  const add = text.match(/^todo\s+tambah\s+([\s\S]+)$/i) || text.match(/^todo\s+(?!lihat$|selesai\s)([\s\S]+)$/i);
  if (add) {
    if (!canManage) return '❌ Anda tidak memiliki akses untuk perintah ini.';
    const todoText = add[1].trim();
    if (!todoText) return 'Format salah. Contoh: todo revisi skripsi';
    const now = nowIso();
  ...
  if (done) {
    ...
    const row = db.prepare('SELECT todo_text FROM todos WHERE id=?').get(id);
    ...
    return `🎉 Yey kamu sudah mengerjakan: ${row ? row.todo_text : 'Tugas #' + no}`;
  }
